...
logger.debug(
    json.dumps(input_list,indent=2, default=str)
)
for item in response.output:
    if item.type == "function_call":
        if item.name == "get_alerts":
            # 3. Execute the function logic for get_horoscope
            logger.debug(f"Executing get_alerts with arguments: {item.arguments}")
        ...
        if item.name == "get_forecast":
            ...
            logger.debug(f"Executing get_forecast with arguments: {item.arguments}")
